...
static tsStatus cpu_bn(const tsTensor* X, const tsTensor* Mean, const tsTensor* Var,
  ...
  const float *x=(const float*)X->ptr, *m=(const float*)Mean->ptr, *v=(const float*)Var->ptr, *g=(const float*)Gamma->ptr, *b=(const float*)Beta->ptr;
  float *y=(float*)Y->ptr;
  long long elems=1; for(int i=0;i<X->rank;i++) elems*=X->shape[i];
  long long rows = elems / C;
  for(long long n=0;n<rows;n++){ long long base=n*C; for(long long c=0;c<C;c++){ float inv=1.0f/std::sqrt(v[c]+eps); y[base+c]=g[c]*(x[base+c]-m[c])*inv+b[c]; } }
  return TS_OK;
}

tsStatus tesseraMatmulAsync(const tsTensor* A, const tsTensor* B, tsTensor* C, tsStream* s){
  (void)s; if(A->device!=-1||B->device!=-1||C->device!=-1) return TS_ERR_BACKEND; return cpu_matmul(A,B,C);
}
tsStatus tesseraBatchNormAsync(const tsTensor* X, const tsTensor* Mean, const tsTensor* Var,
                               ...
                               float eps, tsStream* s){
  (void)s; if(X->device!=-1||Y->device!=-1) return TS_ERR_BACKEND; return cpu_bn(X,Mean,Var,Gamma,Beta,Y,eps);
}
