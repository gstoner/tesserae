...
class MLP(nn.Module):
    def __init__(self,d=16,k=2): super().__init__(); self.net=nn.Sequential(nn.Linear(d,64),nn.ReLU(),nn.Linear(64,k))
    def forward(self,x): return self.net(x)

if __name__ == "__main__":
    torch.manual_seed(0)
    x = torch.randn(32, 16); y = (x[:,0]>0).long()
    m = MLP(16,2); opt = torch.optim.Adam(m.parameters(),1e-3)
    for _ in range(300):
        loss = F.cross_entropy(m(x), y); opt.zero_grad(); loss.backward(); opt.step()
    ig = integrated_gradients(m, x[:4], baseline=torch.zeros_like(x[:4]), steps=64, target_idx=1)
    print("IG attribution shape:", ig.shape)
