...
def tcav_score(model, concept_x, random_x, x_eval, target_idx, layer_fn):
    with torch.no_grad():
        Hc = layer_fn(concept_x)
        Hr = layer_fn(random_x)
    cav = (Hc.mean(0) - Hr.mean(0)); cav = cav / (cav.norm()+1e-8)

    with torch.no_grad():
        H = layer_fn(x_eval)
        eps = 1e-2
        dpos = model.head(H + eps*cav).select(-1, target_idx)
        dneg = model.head(H - eps*cav).select(-1, target_idx)
        dir_deriv = (dpos - dneg) / (2*eps)
        score = (dir_deriv > 0).float().mean().item()
    return score
